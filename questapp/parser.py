...
from .models import Clue, Game, Round, Category
...
def parse_game_html(page, game_id):
    ...
    # Create the game's rounds and categories.
    for round_name in ['jeopardy_round', 'double_jeopardy_round']:
        round_div = bs_game.find('div', {'id': round_name})
        if round_div:
            round = Round.objects.create(game=game, name=round_name)
            cats = parse_round_cats(round, round_div)

            clue_data_list = _parse_round_clues(round, round_div, cats)
            parse_clue_data(clue_data_list, round)
...
def _parse_round_clues(round, round_div, cats):
    # clue_data_list = []
    for row in round_div.table.find_all('tr')[1:]:
        ...
        for i, clue in enumerate(clues):
            ...
            category = Category.objects.get(name=cats[i], round=round)
            if not category:
                continue
            ...
            if valid_clue_data(category=category, question=question, answer=answer):
                Clue.objects.create(category=category, question=question, answer=answer)
...
def valid_clue_data(**kwargs):
    if len(kwargs['question']) < 3:
        return False
    if '<a href' in kwargs['question']:
        # errors.append(HrefException(clue_data['question']))
        log.debug("Skipping a question containing an href.")
        return False
    return True

# def parse_clue_data(clue_data_list, round):
#     errors = []
#     try:
#         for clue_data in clue_data_list:
...
def parse_round_cats(round, round_div):
    cats = []

    cat_row = round_div.table.find_all('tr')[0]
    if not cat_row:
        raise CategoryException("No cat row in round %s" % round)

    cat_els = cat_row.find_all('td', {'class': "category_name"})
    if not cat_els or len(cat_els) < 6:
        raise CategoryException("Expected 6 cat els in %s, got %s" % (round, len(cat_els)))

    for cat_el in cat_els:
        if not cat_el.text:
            # raise CategoryException("No category text in game %s" % game)
            log.warn("No category text in round %s" % round)
            cats.append(None)
            ...
            log.warn("Bad category in round %s, %s" % (round, cat_el.text))
            cats.append(None)
        else:
            cats.append(Category.objects.create(name=cat_el.text, round=round))
...
#         # Get the categories
#         try:
#             cats = parse_round_cats(round_div, game)
#         except CategoryException:
#             raise
#
#         for row in round_div.table.find_all('tr')[1:]:
